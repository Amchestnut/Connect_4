...
class MINIMAXAgent:
    ...
    # Drop the token down in the selected column        
    ...
    def is_terminal_node(self, board):
        ...
        if self.is_winning_move(board, self.player) or self.is_winning_move(board, 2 if self.player == 1 else 1):
            return True
    ...
    def minimax(self, board, depth, alpha, beta, maximizingPlayer):
        ...
        if depth == 0 or is_terminal:
            if is_terminal: # We added here the check if it is a game win/lose move
                if self.is_winning_move(board, 1):                  # 1 is my player, 2 is opposite
                    return 1000
                ...
                return self.score_position(board, 1)
        ...
        if maximizingPlayer:
            ...
            for col in valid_locations:
                ...
                b_copy = board.copy()
                self.drop_token(b_copy, row, col, 1)
                ...
                if alpha >= beta:
                    break 
            return value       # Before it was returning column, value
            ...
            for col in valid_locations:
                row = self.get_next_open_row(board, col)
                b_copy = board.copy()
                self.drop_token(b_copy, row, col, 2)
    ...
    def bestMove(self, board):
        ...
        for col in self.get_possible_moves(board):
            ...
            self.drop_token(temp_board, row, col, self.player)
            score = self.minimax(temp_board, self.num_iterations, -math.inf, math.inf, False)
            if score > best_score:
                best_score = score
                best_col = col
