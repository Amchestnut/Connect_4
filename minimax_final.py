...
class MINIMAXAgentBot:
    ...
    def minimax(self, board, depth, alpha, beta, maximizingPlayer):
        ...
        if depth == 0 or is_terminal:
            if is_terminal:                                         # We added here the check if it is a game win/lose move
                if self.is_winning_move(board, self.player):        # 1 is my player, 2 is opposite
                    return 1000
                elif self.is_winning_move(board, opp_piece):
                    return -2500
                else:
                    return 0
            else:
        ...
        if maximizingPlayer:
            ...
            for col in valid_locations:
                ...
                b_copy = board.copy()
                self.drop_token(b_copy, row, col, self.player)
            ...
            for col in valid_locations:
                row = self.get_next_open_row(board, col)
                b_copy = board.copy()
                self.drop_token(b_copy, row, col, opp_piece)
    ...
    def bestMove(self, board):
        ...
        for col in self.get_possible_moves(board):
            ...
            self.drop_token(temp_board, row, col, self.player)          # Drop the token 1 or 2, depending on the initialized player
